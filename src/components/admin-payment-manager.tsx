...
import { CalendarIcon, PlusCircle, Trash2, Bell, Ban } from 'lucide-react';
import { cn } from '@/lib/utils';
import { format, parseISO } from 'date-fns';
import type { AdminPaymentNotice } from '@/lib/types';
...
type PaymentNoticeFormData = z.infer<typeof paymentNoticeSchema>;

export default function AdminPaymentManager() {
    const { toast } = useToast();
    const { formatCurrency, registerValue, convertedValues } = useCurrency();
    const [notices, setNotices] = React.useState<AdminPaymentNotice[]>([]);
    const [admins, setAdmins] = React.useState<any[]>([]);
    const [isLoading, setIsLoading] = React.useState(true);

    const form = useForm<PaymentNoticeFormData>({
    ...
    const onSubmit = (data: PaymentNoticeFormData) => {
        const newNotice: AdminPaymentNotice = {
            ...data,
    ...
    const removeNotice = (email: string) => {
                        ...
                        <TableBody>
                            ...
                            {notices.map(notice => (
                                <TableRow key={notice.adminEmail}>
                                    ...
                                    <TableCell>
                                        <Button variant="ghost" size="icon" onClick={() => removeNotice(notice.adminEmail)}>
