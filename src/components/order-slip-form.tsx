...
type OrderSlipFormData = z.infer<typeof orderSlipSchema>;

interface OrderSlipFormProps {
    products: Product[];
    onCreate: (orderSlipData: any) => void;
}

export function OrderSlipForm({ products, onCreate }: OrderSlipFormProps) {
    const { toast } = useToast();
    const [productSearch, setProductSearch] = React.useState('');

    const form = useForm<OrderSlipFormData>({
    ...
    const onSubmit = (data: OrderSlipFormData) => {
        if (!data.frame) {
    ...
    const renderPrescriptionInputs = (eye: 'prescription.rightEye' | 'prescription.leftEye') => (
