...
import type { Invoice, Product, PurchaseOrder, Shop } from '@/lib/types';
...
function ReportTableRow({ item, index }: { item: ReportItem, index: number}) {
...
function ReportTable({ data }: { data: ReportItem[] }) {
    if (data.length === 0) {
        return (
...
export default function AuditReport({ detailed = false }: { detailed?: boolean}) {
    ...
    const brands = React.useMemo(() => {
        ...
        return ['all', ...Array.from(brandSet as Set<string>)];
    }, [products]);

    const filteredInvoices = React.useMemo(() => {
        if (selectedShopId === 'all') return invoices;
        return invoices.filter(i => i.shopId === selectedShopId);
    }, [selectedShopId, invoices]);
    
    const salesData = React.useMemo(() => {
        const salesMap = new Map<string, Omit<ReportItem, 'totalValue'> & { totalValue: number }>();
    ...
    const purchaseData = React.useMemo(() => {
        const purchaseMap = new Map<string, ReportItem>();
        const filteredPurchaseOrders = selectedShopId === 'all' ? purchaseOrders : purchaseOrders.filter(p => p.shopId === selectedShopId);
    ...
    }, [selectedBrand, selectedShopId, purchaseOrders]);
